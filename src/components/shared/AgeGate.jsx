...
export function AgeGate({ minAge = 18, label = 'this part of the coven', onPass, onClose }) {
  ...
  return (
    <div className="absolute inset-0 z-[60] flex items-center justify-center p-6 animate-fade-in"
      ...
      {onClose && (
        <button onClick={onClose} className="absolute top-5 right-5 text-[#9E2A33]/60 hover:text-[#C9A961] safe-pt" aria-label="back"><X size={22} /></button>
      )}
      <div className="relative w-full max-w-xs text-center">
        <div className="text-[#9E2A33] text-[10px] uppercase tracking-[0.5em] mb-3" style={F.scriptureSC}>· the door asks ·</div>
        <div className="text-[#C9A961] text-5xl mb-3" style={F.brand}>{minAge}+</div>
        <p className="text-[#9E2A33]/80 text-sm italic mb-6" style={F.scripture}>
          {label} is for those {minAge} and older. enter your date of birth to pass.
        </p>

        <label className="block text-[10px] uppercase tracking-[0.3em] text-[#9E2A33]/70 mb-2" style={F.scriptureSC}>· date of birth ·</label>
