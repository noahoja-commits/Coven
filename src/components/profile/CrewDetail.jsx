      ...
      <div className="border-t border-[#1A1A1A] bg-[#0A0A0A] px-3 py-2 pb-3">
        <div className="flex items-end gap-2">
          ...
          <button onClick={send} disabled={!draft.trim()}
            className={`w-9 h-9 shrink-0 rounded-full flex items-center justify-center transition-colors ${draft.trim() ? 'bg-[#8B0000] text-[#F5F1E8]' : 'bg-[#141414] border border-[#2A2A2A] text-[#6B6B6B]'}`}>
            <Send size={15} />
          </button>
        </div>
      </div>
