...
export function ArcanaCard({ handle, mode = 'compact', onTap }) {
  ...
  if (mode === 'compact') {
    return (
      <button onClick={onTap} title={`${c.numeral} · ${c.name}`}
        ...
        <span className="text-[7px] text-[#A89968] leading-none mt-1" style={F.mono}>{c.numeral}</span>
        <span className="text-lg text-[#C9A961] leading-none my-auto">{c.symbol}</span>
        <span className="text-[5px] uppercase tracking-[0.15em] text-[#A89968]/70 leading-none mb-1 px-0.5 truncate max-w-full" style={F.ui}>{c.name}</span>
      </button>
    );
  }
  ...
  return (
    <div className="relative w-[200px] aspect-[5/8] bg-[#0A0204] border border-[#C9A961]/30 animate-card-flip overflow-hidden flex flex-col items-center justify-center px-5 text-center"
      ...
      <div className="text-[#A89968] text-xs tracking-[0.3em] mb-3" style={F.mono}>{c.numeral}</div>
      ...
      {c.meaning && <div className="text-[#A89968]/80 text-[11px] italic leading-snug" style={F.scripture}>{c.meaning}</div>}
    </div>
  );
}
