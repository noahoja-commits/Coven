  ...
  if (mode === 'compact') {
    return (
      <button onClick={onTap} title={`${c.numeral} · ${c.name}`}
        ...
        <span className="relative text-[7px] text-[#C8102E] leading-none mt-1" style={F.mono}>{c.numeral}</span>
        {!art && <span className="text-lg text-[#C9A961] leading-none my-auto">{c.symbol}</span>}
        <span className={`relative text-[5px] uppercase tracking-[0.15em] leading-none mb-1 px-0.5 truncate max-w-full ${art ? 'text-[#F5F1E8] mt-auto' : 'text-[#C8102E]/70'}`} style={F.ui}>{c.name}</span>
      </button>
    );
  }
  ...
  if (art) {
    return (
      <div className="relative w-[200px] aspect-[5/8] bg-[#0A0204] border border-[#C9A961]/30 animate-card-flip overflow-hidden">
        ...
        <div className="absolute top-3 inset-x-0 text-center text-[#C8102E] text-xs tracking-[0.3em]" style={F.mono}>{c.numeral}</div>
