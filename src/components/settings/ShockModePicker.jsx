...
const SWATCH = {
  ...
  scream: 'radial-gradient(ellipse at center, #1A0408 30%, #C8102E)',
  glitch: 'linear-gradient(180deg, #C8102E 0 20%, #00c8c8 40% 45%, #7B2CBF 70% 78%, #0A0204)',
  inferno: 'radial-gradient(ellipse at 50% 120%, #ff5a00, #C8102E 45%, #0A0204 75%)',
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-black/45 backdrop-blur-[2px] flex flex-col animate-fade-in">
      <div className="flex items-center justify-between px-4 h-[60px] safe-pt">
        <div>
          <div className="text-[10px] uppercase tracking-[0.3em] text-[#C8102E]" style={F.scriptureSC}>· how the dark manifests ·</div>
          ...
          {SHOCK_MODES.map(m => {
            ...
            return (
              <button key={m.id} onClick={() => onPick && onPick(m.id)}
                className={`relative text-left border overflow-hidden transition-all ${active ? 'border-[#C8102E]' : 'border-[#2A2A2A] hover:border-[#5B0F1A]'}`}>
                ...
                <div className="px-2.5 py-2">
                  <div className={`text-sm leading-none ${active ? 'text-[#C8102E]' : 'text-[#F5F1E8]'}`} style={F.displayOrnate}>{m.label}</div>
                  <div className="text-[9px] uppercase tracking-wider text-[#6B6B6B] mt-1" style={F.ui}>{m.desc}</div>
                </div>
                {active && <span className="absolute top-1.5 right-1.5 text-[#C8102E] text-xs">✓</span>}
              </button>
            );
          })}
        </div>
