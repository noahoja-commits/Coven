...
export function CovenMenu({ onClose, onOpen }) {
  return (
    <div className="absolute inset-0 z-30 animate-fade-in"
      ...
      <button onClick={onClose} className="absolute top-4 right-4 z-20 text-[#9E2A33] hover:text-[#C9A961] p-2 -m-1 transition-colors safe-pt"><X size={20} /></button>
      ...
      <div className="absolute inset-0 flex flex-col items-center px-6 py-14 safe-pt safe-pb overflow-y-auto">
        <div className="text-center mb-8 animate-fade-in">
          <div className="text-[#9E2A33] text-[10px] uppercase tracking-[0.5em] mb-3" style={F.scriptureSC}>· enter the ·</div>
          <h1 className="text-[#C9A961] text-5xl mb-3" style={F.brand}>Coven</h1>
          <p className="text-[#9E2A33]/70 text-sm italic max-w-xs mx-auto" style={F.scripture}>"choose your door."</p>
        </div>
            ...
            return (
              <button key={p.id}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[#9E2A33]/70 text-[11px] italic" style={F.scripture}>{p.desc}</p>
                </div>
              </button>
            );
          })}
        </div>
        <div className="mt-8 text-[#9E2A33]/40 text-xs text-center" style={F.scripture}>· more doors will open ·</div>
      </div>
    </div>
  );
}
