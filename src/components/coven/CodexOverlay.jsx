...
export function CodexOverlay({ onClose }) {
  ...
  return (
    <div className="animate-portal-in absolute inset-0 z-30 flex flex-col"
      ...
      <div className="relative bg-[#050204]/95 backdrop-blur-md border-b border-[#A89968]/15 safe-pt">
        <div className="px-4 h-[60px] flex items-center justify-between">
          ...
          <span className="text-[#C8102E]/60 text-[10px] uppercase tracking-wider" style={F.ui}>{filtered.length}</span>
        </div>
        <div className="px-4 pb-3">
          <div className="flex items-center gap-2 px-3 py-2 bg-[#0A0204] border border-[#A89968]/20">
            ...
            <input
              ...
              className="bg-transparent text-[#F5F1E8] text-sm outline-none flex-1 placeholder:text-[#C8102E]/40"
              style={F.scripture}
            />
            {query && (
              <button onClick={() => setQuery('')} className="text-[#C8102E]/60" aria-label="clear">
                <X size={12} />
              </button>
            )}
          </div>
            ...
            <button key={c.id} onClick={() => setCat(c.id)}
              className={`shrink-0 px-3 py-1 text-[10px] uppercase tracking-wider border transition-colors ${
                cat === c.id
                  ...
                  : 'border-[#A89968]/30 text-[#C8102E]/70 hover:border-[#A89968]'
              }`}
      ...
      <div className="relative flex-1 overflow-y-auto px-4 py-3 pb-12 safe-pb">
        {filtered.length === 0 && (
          <div className="text-center py-12 text-[#C8102E]/50 text-xs italic" style={F.scripture}>
            · no entries found ·
          </div>
        )}
          ...
          {filtered.map(e => (
            <article key={e.term} className="border-b border-[#A89968]/15 pb-3">
              <div className="flex items-baseline justify-between gap-3 mb-1">
                ...
                <span className="text-[9px] text-[#C8102E]/50 uppercase tracking-wider shrink-0" style={F.ui}>{e.cat}</span>
              </div>
