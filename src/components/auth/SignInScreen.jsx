      ...
      <div className="relative px-8 flex flex-col items-center justify-center min-h-[100dvh] text-center">
        ...
        {status === 'confirm' || status === 'sent' ? (
          ...
          <form onSubmit={submit} className="w-full max-w-xs animate-fade-in">
            {isReset ? (
              <div className="text-[10px] uppercase tracking-[0.3em] text-[#9E2A33] mb-4" style={F.scriptureSC}>· reset password ·</div>
            ) : (
            ...
            {!isReset && (
              <p className="mt-7 text-[10px] leading-relaxed text-[#6B6B6B]" style={F.ui}>
                ...
                <button type="button" onClick={() => setLegal('terms')} className="text-[#9E2A33] underline">Terms</button>{' '}and{' '}
                <button type="button" onClick={() => setLegal('privacy')} className="text-[#9E2A33] underline">Privacy Policy</button>.
              </p>
            )}
          </form>
        )}
