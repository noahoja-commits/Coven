...
export function StoryComposer({ onClose, onPost }) {
  ...
  const submit = () => {
    ...
    onPost && onPost({ glyph, caption: caption.trim(), bg: bg.id });
    onClose && onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[55] bg-black flex flex-col animate-fade-in">
      ...
      {/* Preview */}
      <div className="relative flex-1 overflow-hidden" style={{ background: bg.bg }}>
      ...
      <div className="bg-black/80 backdrop-blur-md border-t border-white/10 p-4 space-y-3">
        <input
          ...
          className="w-full bg-white/5 border border-white/10 focus:border-white/30 outline-none p-2.5 text-white text-sm placeholder:text-white/40"
          style={F.serif}
          autoFocus
        />
