...
- **A NEW `profiles` column is invisible to clients until granted.** Migration 0026 replaced profiles' table-level SELECT with a column whitelist, so adding a column needs BOTH `grant select (new_col) on public.profiles to authenticated` in the migration (or reads fail with 42501; INSERT/UPDATE grants are still table-level) AND an entry in `PROFILE_COLS_*` in `profiles.js` with a pre-migration fallback tier (see the tos_/mood/decor pattern and 0065).
...
- **Claude auto-mode can't do prod writes** (live posts, `apply.mjs` migrations, Stripe actions) — the permission classifier blocks them unless the human explicitly authorizes in the moment. A human runs (or explicitly green-lights) those; agents prep the exact commands and verify after.
